    ...
    static class Exponential {
        ...
        private long pausedInTotal(int attempts) {
            ...
            for (int i = 0; i < attempts; i++) {
                // There is no point of adding the pause time from the next retry,
                // as it is guaranteed to exceed totalDelayMs
                // and it prevents possible overflow
                if (result > halfTotalMs) {
                    return totalDelayMs;
                }
                result += baseDelayMs * (1L << i);
            }
            return result;
        }
    }
